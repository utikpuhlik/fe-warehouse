    ...
    return (
        <main>
            <div className="mb-4 flex items-center justify-between">
                <Breadcrumbs
                    breadcrumbs={[
                        {label: ORDER_STATUS_LABELS[order.status], href: `/orders?status=${order.status}`},
                        {label: USER_TYPE_LABELS[order.user.customer_type], href: "/orders"},
                        {
                            label: fullName,
                ...
                <div className="flex space-x-2">
                    <DownloadOrderWord orderId={order_id} full={true}/>
                </div>
            </div>

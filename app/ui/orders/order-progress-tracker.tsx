...
import {CheckCircle, CheckCircle2, BellPlus, Package, Truck, BanIcon} from "lucide-react";
...
export function OrderProgressTracker({status, updatedAt}: OrderProgressProps) {
    ...
    const progressValue =
        currentStepIndex === -1 ? 0 : (currentStepIndex / (progressStatuses.length - 1)) * 100;

    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center">{dict.deliveryInfo}</CardTitle>
            </CardHeader>
            <CardContent>

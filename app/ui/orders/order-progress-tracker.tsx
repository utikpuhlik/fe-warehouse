import { BanIcon, BellPlus, CheckCircle, CheckCircle2, Package, Truck } from "lucide-react";
...
const progressStatuses: Exclude<OrderStatusEnum, "CANCELED">[] = ["NEW", "IN_PROGRESS", "SHIPPING", "COMPLETED"];

function getStatusLabels(t: (key: string) => string): Record<Exclude<OrderStatusEnum, "CANCELED">, string> {
  return {
    NEW: t("new"),
...
export function OrderProgressTracker({ status, updatedAt }: OrderProgressProps) {
  ...
  const progressValue = currentStepIndex === -1 ? 0 : (currentStepIndex / (progressStatuses.length - 1)) * 100;

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center">{t("delivery_info")}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="relative space-y-6 pt-1">
          <div className="mb-2 flex items-center justify-between">
            {progressStatuses.map((step, index) => {
              ...
              const icon = isCompleted ? <CheckCircle2 className="size-5" /> : statusIcons[step];

              const circleClass =
                isCompleted || isCurrent ? "bg-green-600 text-white dark:bg-green-900" : "bg-muted border";
          ...
          <div className="space-y-6">
            <Progress className="w-full" value={progressValue} color="bg-green-200 dark:bg-green-800" />

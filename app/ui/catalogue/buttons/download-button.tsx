...
export function DownloadButton({ onClick, full = true, disabled = false, loading = false }: DownloadButtonProps) {
    return (
        <Button
            variant="outline"
            ...
            disabled={disabled}
        >
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <ArrowDownTrayIcon className="h-5 w-5" />}
